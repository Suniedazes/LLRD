...
test("contact validates required fields, formats, enums and lengths",()=>{assert.deepEqual(validateInquiry(valid).errors,{});assert.equal(Object.keys(validateInquiry({}).errors).length,5);assert(validateInquiry({...valid,email:"bad",service:"bad",message:"x".repeat(5001)}).errors.email);assert(validateInquiry({...valid,message:"x".repeat(5001)}).errors.message);});
