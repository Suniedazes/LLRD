...
export const contactOptions=[...services.map(s=>s.value),"Other"];
export type Inquiry={name:string;organization:string;email:string;service:string;message:string;phone:string;preferred:string;website:string};
export function validateInquiry(input:unknown){
 ...
 const limits={name:100,organization:160,email:254,service:80,message:5000,phone:40,preferred:30,website:200};
 ...
 for(const key of ["name","organization","email","service","message"] as const)if(!data[key])errors[key]="This field is required.";
 ...
 if(data.preferred&&!["Email","Phone","Video Meeting"].includes(data.preferred))errors.preferred="Choose a contact preference.";
 return {data,errors};
}
